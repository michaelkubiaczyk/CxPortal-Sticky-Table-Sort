    ...
    <script type="text/javascript">
        ...
        function compareScans(e, isConfirmed) {            
			...
			if (scansIDs.length != 2) {
                ...
                var oWnd = radopenExt("popScanCompare.aspx?id1=" + newScan.scanId + "&id2=" + oldScan.scanId + "&pid1=" + newScan.projectId + "&pid2=" + oldScan.projectId);
        ...
        function openReport(i_Type, i_ScanID, i_ProjectID, i_ProjectName, i_ScanDate) {
            if (i_Type != "VIEWER") {
                var win = radopenExt("GenerateScanReportNew.aspx?scanid=" + i_ScanID + "&projectid=" + i_ProjectID + "&ProjectName=" + encodeURIComponent(i_ProjectName));
        ...
        function openSummery(i_ScanID, i_ProjectName, i_ProjectID, ScanCompletedStatus) {
            var oWnd = radopenExt("popScanSummery.aspx?ProjectId=" + i_ProjectID + "&ScanID=" + i_ScanID + "&ProjectName=" + "&rnd=" + Math.random());
            var height = 895;
        ...
        <div class="box-content" style="min-height: 600px; padding-right: 2px;">
            <CX:CxGrid ID="ScansGrid" runat="server" ShowGroupPanel="true" AllowSorting="True"
                ...
                <MasterTableView AutoGenerateColumns="false" DataKeyNames="ScanID, ProjectID" ClientDataKeyNames="ScanID, ProjectID, IsLocked"
                    ...
                    <Columns>
                        <telerik:GridTemplateColumn HeaderStyle-Width="70" AllowFiltering="True" DataField="ScanScope" UniqueName="ScanScope" Resizable="false" Groupable="False">
                            <ItemStyle VerticalAlign="Middle" HorizontalAlign="Center" />
                            <FilterTemplate>
                                <telerik:RadComboBox CssClass="ScanScopeCombo" ID="cmbScanScope" Width="105%" AppendDataBoundItems="true" DropDownWidth="150px"
                        ...
                        <telerik:GridTemplateColumn DataField="Origin" UniqueName="Origin" SortExpression="Origin"
                            AllowFiltering="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains">
                            <ItemTemplate>
                                <code ng-non-bindable>
                                    <%#this.EncodeCurrentField(DataBinder.Eval(Container.DataItem, "Origin").ToString())%>
                                </code>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>
                        <telerik:GridTemplateColumn DataField="OriginURL" UniqueName="OriginURL" SortExpression="OriginURL"
                            AllowFiltering="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains">
                            <ItemTemplate>
                                <asp:HyperLink ng-non-bindable runat="server" Text=<%#this.EncodeCurrentField(DataBinder.Eval(Container.DataItem, "OriginURL").ToString())%>
                                    Target="_blank" NavigateUrl=<%#this.EncodeCurrentField(DataBinder.Eval(Container.DataItem, "OriginURL").ToString())%>>
                                </asp:HyperLink>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>
                        ...
                        <telerik:GridTemplateColumn DataField="TeamName" UniqueName="TeamName"
                            SortExpression="TeamName" AllowFiltering="true" AutoPostBackOnFilter="true"
                            CurrentFilterFunction="Contains">
                            <ItemTemplate>
                                <code ng-non-bindable>
                                    <%#this.EncodeCurrentField(DataBinder.Eval(Container.DataItem, "TeamName").ToString())%>
                                </code>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>
