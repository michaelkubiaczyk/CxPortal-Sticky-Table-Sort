<%@ Page Title="" Language="C#" MasterPageFile="~/MainMaster.master" AutoEventWireup="true" Inherits="ProjectScans" CodeBehind="ProjectScans.aspx.cs" %>
...
<style type="text/css">
        ...
        function onRequestStart(sender, args) {
            ...
            if (args.get_eventTarget().indexOf("_GridToolbar") >= 0) {
                if (toolbarBtn.get_value() != "REFRESH") {
                    if (toolbarBtn.get_value() != '<%=k_ScanNowValue%>' && toolbarBtn.get_value() != '<%=k_IncrementalScan%>') {
    ...
    function deleteScans(isConfirmed, flags) {
            ...
            $telerik.$.ajax({
                type: 'POST',
                async: false,
                url: 'ProjectScans.aspx/DeleteScans',
        ...
        function compareScans(e) {
            ...
            if (scansIDs.length != 2) {
                ...
                var oldScan = parseInt(scansIDs[0]) > parseInt(scansIDs[1]) ? scansIDs[1] : scansIDs[0];
                var newScan = parseInt(scansIDs[0]) > parseInt(scansIDs[1]) ? scansIDs[0] : scansIDs[1];

                var oWnd = radopen("popScanCompare.aspx?id1=" + newScan + "&id2=" + oldScan + "&pid1=<%=m_CurrentProjectId%>&pid2=<%=m_CurrentProjectId%>");
                oWnd.setSize(1365, 883);
                oWnd.center();
                oWnd.show();
            }
        }
        ...
        function ConfirmWindow(i_Msg, i_Event) {
         ...
         function openReport(i_Type, i_ScanID, i_ProjectID, i_ScanDate) {
             if (i_Type != "VIEWER") {
                 var win = radopen("GenerateScanReportNew.aspx?scanid=" + i_ScanID + "&projectid=" + i_ProjectID + "&ProjectName=" + encodeURIComponent('<%=HttpUtility.UrlEncode(ProjectName).Replace("'", "\\'")%>'));
...
<asp:Content ID="cnt2" ContentPlaceHolderID="cpmain" runat="Server">  
    ...
    <div class="box">        
        <div class="box-content" style="min-height: 600px;padding-right:2px;">
            ...
            ExportHiddenColumnsList="Compare,Action,ScanLogs" MultiSelectionCheckboxes="true"
                AllowPaging="true" AllowFilteringByColumn="True" EnableViewState="true" EnableLinqExpressions="false">
                <MasterTableView AutoGenerateColumns="false" DataKeyNames="ScanID" ClientDataKeyNames="ScanID,IsLocked"
                    GroupLoadMode="Client" ShowHeadersWhenNoRecords="true" EnableNoRecordsTemplate="true"
                    AllowAutomaticInserts="false">
                    <Columns>
                        <telerik:GridTemplateColumn HeaderStyle-Width="35" AllowFiltering="True" DataField="ScanScope" UniqueName="ScanScope" Groupable="False">
                        ...
                        <telerik:GridDateTimeColumn DataField="ScanFinishDate" UniqueName="ScanFinishDate"
                            SortExpression="ScanFinishDate" AllowFiltering="true" DataType="System.DateTime"
                            AutoPostBackOnFilter="true" CurrentFilterFunction="LessThanOrEqualTo">
                        </telerik:GridDateTimeColumn>
                        <telerik:GridNumericColumn DataField="ScanID" Visible="false" UniqueName="ScanID"
                            SortExpression="ScanID" AllowFiltering="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains">
                        </telerik:GridNumericColumn>
                        <telerik:GridBoundColumn DataField="ProjectName" Visible="false" UniqueName="ProjectName"
                            SortExpression="ProjectName" AllowFiltering="true" AutoPostBackOnFilter="true"
                        ...
                        <telerik:GridBoundColumn DataField="InitiatorName" UniqueName="InitiatorName"
                            SortExpression="InitiatorName" AllowFiltering="true" AutoPostBackOnFilter="true"
                            CurrentFilterFunction="Contains">
                        </telerik:GridBoundColumn>
                        <telerik:GridBoundColumn DataField="Origin" UniqueName="Origin"
                            SortExpression="Origin" AllowFiltering="true" AutoPostBackOnFilter="true"
                            CurrentFilterFunction="Contains">
                        </telerik:GridBoundColumn>
                        ...
                        <telerik:GridTemplateColumn UniqueName="Access" SortExpression="Access" DataField="Access" 
                            AllowFiltering="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains">
                            <ItemTemplate>
                        ...
                        <telerik:GridTemplateColumn UniqueName="IsLocked" SortExpression="IsLocked" DataField="IsLocked" 
                            AllowFiltering="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains">
                            <ItemTemplate>
                                <asp:Label runat="server" Text='<%# Convert.ToBoolean(Eval("IsLocked")) == true ? GetTermFromResource("LOCKED") : string.Empty %>'></asp:Label>
                            </ItemTemplate>
                        </telerik:GridTemplateColumn>
                        <telerik:GridTemplateColumn HeaderStyle-Width="100px" UniqueName="Action" Groupable="False"
                            AllowFiltering="false" ItemStyle-CssClass="actionbuttons" HeaderStyle-CssClass="actionbuttons">
                            <ItemTemplate>
                                <div style="vertical-align: middle; cursor: pointer;">
                                    ...
                                    <asp:ImageButton runat="server" Visible='<%# PortalUser.IsAllowedToDeleteScan %>' ImageUrl="images/icons/unlocked.png" ToolTip='<%# GetTermFromResource("LOCK_SCAN") %>' CssClass='<%# Convert.ToString(Eval("Access")) == "PUBLIC" && Convert.ToBoolean(Eval("IsLocked")) == false ? "availableAction" : "unavailableAction" %>' OnClientClick='<%# ("lockScan(" + Eval("ScanID") + ", refresh)") %>' />
                                    <asp:ImageButton runat="server" Visible='<%# PortalUser.IsAllowedToDeleteScan %>' ImageUrl="images/icons/locked.png" ToolTip='<%# GetTermFromResource("UNLOCK_SCAN") %>' CssClass='<%# Convert.ToString(Eval("Access")) == "PUBLIC" && Convert.ToBoolean(Eval("IsLocked")) == true ? "availableAction" : "unavailableAction" %>' OnClientClick='<%# ("unlockScan(" + Eval("ScanID") + ", refresh)") %>' />
                                </div>
                            </ItemTemplate>
                            <HeaderStyle Width="100px"></HeaderStyle>
                        </telerik:GridTemplateColumn>
                        <telerik:GridTemplateColumn HeaderStyle-Width="35" AllowFiltering="false" UniqueName="ScanLogs" Groupable="False" ItemStyle-CssClass="actionbuttons" HeaderStyle-CssClass="actionbuttons">
                            <ItemStyle VerticalAlign="Middle" HorizontalAlign="Center" />
                            <ItemTemplate>
                                <CX:CxLogsDownloadButton  ID="lbScanLogs" runat="server" Visible='<%# (this.Page as CxPortalPage).PortalUser.IsScanner %>' StreamLogData-ScanType="Finished" StreamLogData-Id='<%# (long)DataBinder.Eval(Container.DataItem, "ScanID") %>' StreamLogData-ProjectName='<%# HttpUtility.UrlEncode(ProjectName) %>' StreamLogData-ScanTime='<%# DataBinder.Eval(Container.DataItem, "ScanFinishDate") %>'>
                ...
                <ExportSettings Pdf-PaperSize="A4" Pdf-PageTitle="Scans" FileName="Scans" Pdf-PageWidth="297mm" Pdf-PageHeight="210mm">
                </ExportSettings>
            </CX:CxGrid>
            <br />
                ...
                <asp:Panel ID="pnlDetails" runat="server">
                    ...
                    <telerik:RadMultiPage ID="ScanMultiPage" runat="server" SelectedIndex="0">
                        <telerik:RadPageView ID="MonitorPagev" runat="server">
                            <div class="DetailsContentContainer" style="height: 305px;">
                        ...
                        <telerik:RadPageView ID="CmmtsPagev" runat="server">
                            <div class="DetailsContentContainer" style="height: 230px; min-height: 100px; border-bottom: none;">
                                <div style="padding: 20px">
                                    <asp:TextBox ID="txtCmmts" runat="server" Width="605" Height="180" TextMode="MultiLine" />
                                </div>
                            </div>
                            <div class="DetailsContentContainer" style="background-color: #eeeeee; min-height: 33px; padding-left: 25px;">
